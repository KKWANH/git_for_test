...
def		display(request):
		try:
			...
			with _con.cursor() as _cur:
				_cur.execute(_sql)
				_mvs = _cur.fetchall()
			return	render(request, 'ex04/display.html', {"movies": _mvs})
		except Exception as _exc:
	...
	def	get(self, request):
		try:
			...
			with _con.cursor() as _cur:
				_cur.execute(_sql)
				_mvs = _cur.fetchall()
			_cxt = {'form': RemoveForm(choices=((_mov[0], _mov[0]) for _mov in _mvs))}
			return	render(request, 'ex04/remove.html', _cxt)
		except Exception as _exc:
			return	HttpResponse("No data available")
	...
	def	post(self, request):
		try:
			...
			with _con.cursor() as _cur:
				_cur.execute(_sql)
				_mvs = _cur.fetchall()
			_cho = ((_mov[0], _mov[0]) for _mov in _mvs)
			_dat = RemoveForm(_cho, request.POST)
			_sql = """
