...
class	Update(View):
	...
	def	get(self, request):
		...
		try:
			with self._con.cursor() as _cur:
				_cur.execute(_sql)
				_mvs = _cur.fetchall()
			_ctx = {'form': UpdateForm(choices=((_mov[0], _mov[0]) for _mov in _mvs))}
	...
	def	post(self, request):
		...
		if _dat.is_valid() == True:
			try:
				with self._con.cursor() as _cur:
					_cur.execute(_sql, [_dat.cleaned_data['opening_crawl'], _dat.cleaned_data['title']])
				self._con.commit()
			except Exception as _exc:
				print(_exc)
